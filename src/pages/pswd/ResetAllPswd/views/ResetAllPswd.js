...
import {getCodeForFindPswd, resetAllPswd} from '@/api'
...
class ResetAllPswd extends Component {
  ...
  async loadCode() {
    ...
    try {
      const {data} = await getCodeForFindPswd(this.props.phone)
      if(data.status === 200) {
  ...
  handleSubmit(e) {
    ...
    const params = {
      newLoginPwd: this.state.newPswd,
